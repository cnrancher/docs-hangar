...
The `hangar save` command is similar to the [mirror](/docs/v1.8/mirror/mirror) command. It allows you to copy multiple container images into an archive file for [load](/docs/v1.8/load/load) command use.
...
3. If error occured when saving some images, the save failed images will output to `save-failed.txt` by default.
    ...
    You can use the `--failed` option to specify the output file of the save failed images, and then use hangar [sync](/docs/v1.8/sync/sync) command to re-download the save failed images in `save-failed.txt` to the archive file without re-creating the whole archive file again.
...
For more information about hangar archive usage, see [load](/docs/v1.8/load/load) and [archive](/docs/v1.8/archive) page.
