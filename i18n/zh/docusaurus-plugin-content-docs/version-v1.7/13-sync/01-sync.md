...
`hangar sync` 命令与 [save](/v1.7/save/save) 命令类似，允许您将额外的镜像附加到 [save](/v1.7/save/save) 命令创建的压缩包中，而不需要重新下载所有的镜像创建新的压缩包文件。

[save](/v1.7/save/save) 命令在创建压缩包时，可能会遇到镜像下载失败等情况，此时您可使用 `sync` 命令将失败的镜像尝试附加到已创建的压缩包中。

## 快速上手

使用以下命令，向 [save](/v1.7/save/save#快速上手) 命令创建的压缩包中增添新的镜像:
