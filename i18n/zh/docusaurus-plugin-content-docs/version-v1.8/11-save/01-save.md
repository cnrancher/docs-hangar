...
`hangar save` 命令与 [mirror](/docs/v1.8/mirror/mirror) 命令类似，用于将容器镜像从镜像仓库下载至本地的压缩文件中，之后可使用 [load](/docs/v1.8/load/load) 命令将压缩文件中的镜像上传到镜像仓库中。
...
3. 如果某些镜像无法下载，下载失败的镜像列表将保存在 `save-failed.txt`。
    ...
    您可使用 `--failed` 参数指定输出的保存失败的镜像列表文件名。之后使用 Hangar 的 [Sync](/docs/v1.8/sync/sync) 命令，将保存失败的镜像重新下载增添在已有的压缩包文件中。
...
关于压缩文件的更多信息，请参考 [load](/docs/v1.8/load/load) 命令和 [压缩文件格式](/docs/v1.8/archive/specification) 页面。
